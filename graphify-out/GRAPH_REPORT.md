# Graph Report - butterflyguy  (2026-06-03)

## Corpus Check
- 122 files · ~290,281 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1880 nodes · 3160 edges · 129 communities (115 shown, 14 thin omitted)
- Extraction: 73% EXTRACTED · 27% INFERRED · 0% AMBIGUOUS · INFERRED: 838 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `eeeb4822`
...
3. `MinuteBar` - 42 edges
4. `make_order_manager()` - 41 edges
5. `SimulationEngine` - 41 edges
6. `SimulationParams` - 40 edges
7. `ButterflyBuilder` - 39 edges
8. `StrategySettings` - 38 edges
9. `ButterflySelector` - 36 edges
10. `OptionQuote` - 34 edges
...
Cohesion: 0.06
Nodes (49): BaseModel, BaseSettings, AppConfig, CollectorSettings, DatabaseSettings, EntrySettings, ExecutionSettings, load_config() (+41 more)
...
Cohesion: 0.05
Nodes (44): chain_cache_path(), ChainDay, load_chain_day(), nearest_snapshot(), Real option chain cache — per-day JSON snapshots from the live collector.  Forma, Load all chain snapshots for a day.      Returns dict of UTC datetime -> list[Op, Return quotes from the most recent snapshot at or before bar_ts., dict of {UTC datetime: OptionQuote list} with a pre-sorted key index for O(log n (+36 more)

### Community 4 - "Community 4"
Cohesion: 0.09
Nodes (40): backtest_entry_price(), candidate_from_trade_row(), day_with_monitoring_bars(), _dd_schedule_label(), _duration_min(), _find_bar_at(), _find_entry_bar_at(), _format_et() (+32 more)
...
Cohesion: 0.09
Nodes (29): ButterflyCandidate, OptionQuote, Pydantic models for option data and trade records., A single option quote from a chain snapshot., A butterfly spread candidate identified by the scanner., EntryDecision, find_entry_candidate(), LiveSpread (+21 more)

### Community 8 - "Community 8"
Cohesion: 0.08
Nodes (31): _as_float(), _as_int(), Convert option_chain_snapshots rows into OptionQuote objects., Build OptionQuote list from option_chain_snapshots query rows., rows_to_option_quotes(), Compare Schwab selection with the nearest DB collector snapshot., EntrySelectionResult, build_entry_selection_parity() (+23 more)
...
Cohesion: 0.13
Nodes (20): DayResult, _drawdown_rule(), DrawdownWindow, _profit_exit_reason(), Single-day simulation engine using synthetic option chains., Simulate intraday using BS pricing, pinned to a pre-selected real entry., Simulate intraday using BS pricing, pinned to a pre-selected real entry., Simulate intraday using BS pricing, pinned to a pre-selected real entry. (+12 more)

### Community 24 - "Community 24"
Cohesion: 0.11
Nodes (17): _chain_spot_price(), PositionService, Position monitoring service — runs state machine on open positions., Monitors open positions and manages exits via the profit state machine., Record trade exit metrics and update risk engine., Monitors open positions and manages exits via the profit state machine., Extract the three butterfly leg quotes from the chain for position valuation., Record trade exit metrics and update risk engine. (+9 more)
...
Cohesion: 0.15
Nodes (13): DayData, Backtest data loader — fetches SPX 1-min bars and VIX from Polygon.io., Runs full strategy on a single day using synthetic options., Runs full strategy on a single day using synthetic options., Runs full strategy on a single day using synthetic options., SimulationEngine, Synthetic option chain generator using Black-Scholes + VIX IV model., Generates a synthetic SPX option chain from spot + VIX. (+5 more)
...
Cohesion: 0.12
Nodes (16): Execute SQL migration files in order., Run all SQL migration files in order., Run all SQL migration files in order., run_migrations(), daily_reset_loop(), entry_loop(), main(), Main orchestrator: runs collector + trading + position monitor concurrently. (+8 more)
...
Cohesion: 0.12
Nodes (12): Simulate one trading day., Simulate one trading day., Simulate one trading day., Classify regime then delegate to simulate_day() with matching params.          R, Classify regime then delegate to simulate_day() with matching params.          R, Classify regime then delegate to simulate_day() with matching params.          R, Classify regime then delegate to simulate_day() with matching params.          R, Paper trading commission: 4 legs × quantity × rate. (+4 more)
...
Cohesion: 0.14
Nodes (9): Queries for daily_risk_state table., Sum of realized PnL for the rolling 7-day window (closed trades only)., Queries for daily_risk_state table., Sum of realized PnL for the rolling 7-day window (closed trades only)., PnL of the last N closed trades (most recent first), for consecutive loss detect, Queries for daily_risk_state table., Sum of realized PnL for the rolling 7-day window (closed trades only)., PnL of the last N closed trades (most recent first), for consecutive loss detect (+1 more)

### Community 48 - "Community 48"
Cohesion: 0.13
Nodes (8): DatabasePool, Async database connection pool using asyncpg., Manages an asyncpg connection pool for TimescaleDB., Create the connection pool., CandidateQueries, Queries for butterfly_candidates table., Queries for butterfly_candidates table., Queries for butterfly_candidates table.
...
Cohesion: 0.17
Nodes (12): _bucket_sigmas(), Return sigma anchors spanning narrow to wide for the bucket size., Return (widths, sigma_fractions) for the active VIX bucket.      Buckets are pro, Return (widths, sigma_fractions) for the active VIX bucket.      Buckets are pro, resolve_wing_widths_for_vix(), _active_widths_and_sigmas(), Shared entry selection for live trading and backtests., Resolve the widths to scan and their positional VIX sigmas. (+4 more)
...
Cohesion: 0.15
Nodes (10): DailyBarQueries, Queries for daily_bars table., Upsert daily OHLCV rows. Updates close/open/high/low/volume on conflict., Return the last `days` daily closes in chronological order (oldest first)., Queries for daily_bars table., Upsert daily OHLCV rows. Updates close/open/high/low/volume on conflict., Return the last `days` daily closes in chronological order (oldest first)., Queries for daily_bars table. (+2 more)
...
Cohesion: 0.21
Nodes (11): get_0dte_expiration(), minutes_since_open(), minutes_to_close(), now_eastern(), now_pacific(), Market timezone helpers for 0-DTE trading., Current time in US/Eastern., Current time in US/Pacific. (+3 more)

### Community 64 - "Community 64"
Cohesion: 0.24
Nodes (4): DiscordNotifier, Trading and risk notifications., Sends trading notifications to Discord via webhook., Sends trading notifications to Discord via webhook.
...
Cohesion: 0.31
Nodes (9): Return the first regular-session open for the requested Eastern date., Return the first regular-session open for the requested Eastern date., Return the first regular-session open for the requested Eastern date., Return the first regular-session open for the requested Eastern date., _session_open_from_intraday_candles(), _candle(), test_session_open_ignores_premarket_and_missing_open_values(), test_session_open_returns_none_when_no_regular_session_bar_exists() (+1 more)

### Community 75 - "Community 75"
Cohesion: 0.2
Nodes (9): A trade record for tracking entry/exit., TradeRecord, Full entry flow from eligibility checks through entry fill., Full entry flow from eligibility checks through entry fill., Full entry flow from eligibility checks through entry fill., Full entry flow from eligibility checks through entry fill., Full entry flow: time → balance → risk → direction (once) → retry loop (re-scan, entry_strategy_snapshot() (+1 more)
...
Cohesion: 0.22
Nodes (7): Trade service — orchestrates entry flow., Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow., TradeService
...
Cohesion: 0.33
Nodes (4): Queries for tent_boundaries table., Queries for tent_boundaries table., Queries for tent_boundaries table., TentQueries

### Community 93 - "Community 93"
Cohesion: 0.33
Nodes (4): DecisionQueries, Queries for decision_log table., Queries for decision_log table., Queries for decision_log table.
...
- **Why does `OptionQuote` connect `Community 7` to `Community 0`, `Community 1`, `Community 2`, `Community 3`, `Community 68`, `Community 6`, `Community 39`, `Community 8`, `Community 13`, `Community 78`, `Community 21`, `Community 24`, `Community 25`, `Community 58`, `Community 27`, `Community 28`, `Community 30`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `run_single()` connect `Community 4` to `Community 33`, `Community 34`, `Community 3`, `Community 35`, `Community 36`, `Community 71`, `Community 40`, `Community 41`, `Community 72`, `Community 12`, `Community 18`, `Community 21`, `Community 54`, `Community 53`, `Community 30`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `ButterflyCandidate` connect `Community 7` to `Community 0`, `Community 32`, `Community 2`, `Community 36`, `Community 4`, `Community 38`, `Community 8`, `Community 78`, `Community 79`, `Community 18`, `Community 20`, `Community 87`, `Community 23`, `Community 24`, `Community 27`, `Community 28`, `Community 30`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Are the 46 inferred relationships involving `str` (e.g. with `make_chain_data()` and `make_chain_data_with_spread()`) actually correct?**
