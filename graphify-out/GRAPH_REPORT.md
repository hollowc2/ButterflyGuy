# Graph Report - butterflyguy  (2026-05-29)

## Corpus Check
- 112 files · ~285,796 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1684 nodes · 2851 edges · 121 communities (106 shown, 15 thin omitted)
- Extraction: 73% EXTRACTED · 27% INFERRED · 0% AMBIGUOUS · INFERRED: 779 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4c3836f6`
...
4. `SimulationEngine` - 40 edges
5. `MinuteBar` - 40 edges
6. `ButterflyBuilder` - 39 edges
7. `SimulationParams` - 37 edges
8. `StrategySettings` - 36 edges
9. `ButterflySelector` - 36 edges
10. `ButterflyCandidate` - 32 edges
...
- `make_settings()` --calls--> `ExecutionSettings`  [INFERRED]
  tests/test_order_manager.py → src/butterfly_guy/core/config.py
- `make_quote()` --calls--> `OptionQuote`  [INFERRED]
  tests/test_order_manager.py → src/butterfly_guy/data/schemas.py
- `make_candidate()` --calls--> `ButterflyCandidate`  [INFERRED]
  tests/test_order_manager.py → src/butterfly_guy/data/schemas.py
- `make_order_manager()` --calls--> `OrderManager`  [INFERRED]
  tests/test_order_manager.py → src/butterfly_guy/execution/order_manager.py
...
Cohesion: 0.08
Nodes (30): DayData, MinuteBar, Backtest data loader — fetches SPX 1-min bars and VIX from Polygon.io., Runs full strategy on a single day using synthetic options., Runs full strategy on a single day using synthetic options., SimulationEngine, SimulationParams, BiasScoreFilter (+22 more)
...
Cohesion: 0.08
Nodes (37): ButterflyCandidate, OptionQuote, A single option quote from a chain snapshot., A butterfly spread candidate identified by the scanner., _butterfly_value(), _compute_spread(), detect_complete_days(), _elapsed() (+29 more)
...
Cohesion: 0.07
Nodes (29): load_chain_day(), Load all chain snapshots for a day.      Returns dict of UTC datetime -> list[Op, SweepConfig, DayResult, _drawdown_rule(), DrawdownWindow, _profit_exit_reason(), Single-day simulation engine using synthetic option chains. (+21 more)

### Community 7 - "Community 7"
Cohesion: 0.09
Nodes (31): BaseSettings, AppConfig, load_config(), Load configuration from YAML file and environment variables., Load configuration from YAML file and environment variables., OptionChainCollector, Option chain collector — fetches and stores SPX chain snapshots., Collects option chain snapshots at regular intervals. (+23 more)
...
Cohesion: 0.12
Nodes (23): candidate_from_trade_row(), _duration_min(), _format_et(), get_vix_at(), load_asset_config(), load_live_trades(), main(), nearest_snapshot() (+15 more)
...
Cohesion: 0.08
Nodes (26): _force_synthetic_for_date(), _patch_chain_cache(), Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable., Patch load_chain_day to return None for `date`, forcing BS synthetic fallback., Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable. (+18 more)

### Community 18 - "Community 18"
Cohesion: 0.09
Nodes (21): Full entry flow from eligibility checks through entry fill., Full entry flow from eligibility checks through entry fill., Full entry flow from eligibility checks through entry fill., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Parse Schwab chain response into OptionQuote objects., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter. (+13 more)
...
Cohesion: 0.11
Nodes (20): main(), parse_args(), print_help(), Inspect what the strategy saw at entry for a given date.  Replicates the synthet, print_day_header(), EntryDecision, find_entry_candidate(), Find best candidate in the 10:00–10:30 ET window, returning full decision contex (+12 more)
...
Cohesion: 0.19
Nodes (19): StrategySettings, Walk the 10:00-10:30 ET window and record every bar's entry signals., scan_entry_window(), ButterflyBuilder, Builds and scores butterfly spreads from an option chain snapshot., Builds and scores butterfly spreads from an option chain snapshot., make_chain(), make_quote() (+11 more)
...
Cohesion: 0.1
Nodes (20): get_prev_close(), load_monitoring_chains(), Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly). (+12 more)

### Community 27 - "Community 27"
Cohesion: 0.13
Nodes (20): _dd_schedule_label(), discover_dates(), _find_bar_at(), _find_entry_bar_at(), _live_width_label(), _print_comparison_table(), _pst_to_et(), All dates in [start, end] with >= 50 snapshots for `underlying`. (+12 more)
...
Cohesion: 0.14
Nodes (15): get_0dte_expiration(), get_time_regime(), minutes_since_open(), minutes_to_close(), now_eastern(), now_pacific(), Market timezone helpers for 0-DTE trading., Current time in US/Eastern. (+7 more)

### Community 32 - "Community 32"
Cohesion: 0.12
Nodes (16): Cross-width selection., Cross-width selection., Cross-width selection., Cross-width selection., Cross-width selection., Cross-width selection., Cross-width selection., Cross-width selection. (+8 more)
...
Cohesion: 0.21
Nodes (13): fmt_candidate(), load_bars_from_db(), load_chains_from_db(), main(), nearest_snapshot(), parse_args(), parse_date(), print_entry_scan() (+5 more)
...
Cohesion: 0.13
Nodes (15): _print_same_entry_comparison_table(), Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday (+7 more)

### Community 40 - "Community 40"
Cohesion: 0.13
Nodes (15): print_thinkback_checklist(), Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist. (+7 more)

### Community 41 - "Community 41"
Cohesion: 0.13
Nodes (15): _asset_drawdowns(), Resolve the DB connection string for local backtests.      Backtests follow the, Resolve the DB connection string for local backtests.      Backtests follow the, Resolve the DB connection string for local backtests.      Backtests follow the, Return live morning/late/afternoon drawdown thresholds., Return live morning/late/afternoon drawdown thresholds., Return live morning/late/afternoon drawdown thresholds., Return live morning/late/afternoon drawdown thresholds. (+7 more)
...
Cohesion: 0.15
Nodes (11): A trade record for tracking entry/exit., TradeRecord, PositionManager, Tracks position value from chain data and manages peak tracking., Reset for a new position. Optionally restore a persisted peak (e.g. after restar, Tracks position value from chain data and manages peak tracking., Reset for a new position. Optionally restore a persisted peak (e.g. after restar, PositionService (+3 more)
...
Cohesion: 0.2
Nodes (11): fly_settlement_value(), Butterfly cash-settlement value from the underlying index close., Record trade exit metrics and update risk engine., Record trade exit metrics and update risk engine., Monitor position every 10s, evaluate state machine, trigger exit if needed., Monitor position every 10s, evaluate state machine, trigger exit if needed., make_candidate(), Tests for butterfly position valuation helpers. (+3 more)

### Community 47 - "Community 47"
Cohesion: 0.14
Nodes (14): _print_pnl_histogram(), ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets. (+6 more)
...
Cohesion: 0.16
Nodes (12): compute_tent_boundaries(), fly_bid_value(), PositionState, Position value tracking and management., Calculate current butterfly value from latest chain quotes.         Value = lowe, Calculate current butterfly value from latest chain quotes.         Value = lowe, Butterfly value at market bid (what a MM pays to buy it from you)., Butterfly value at market bid (what a MM pays to buy it from you). (+4 more)
...
Cohesion: 0.29
Nodes (11): make_risk_engine(), Tests for the risk engine., Should block trading when market is closed., test_can_trade_blocks_quantity_above_max_position_size(), test_can_trade_halted(), test_can_trade_market_closed(), test_can_trade_max_loss(), test_can_trade_max_trades() (+3 more)
...
Cohesion: 0.17
Nodes (6): Risk management engine — enforces daily limits and trading rules., Record that a trade was executed., Overwrite realized_pnl in risk state (SET, not ADD).         Used at startup to, Manually sync the trade count in the risk state table.         Used at startup t, Enforces risk constraints before allowing trades., RiskEngine
...
Cohesion: 0.2
Nodes (10): _bucket_sigmas(), Return sigma anchors spanning narrow to wide for the bucket size., Return (widths, sigma_fractions) for the active VIX bucket.      Buckets are pro, Return (widths, sigma_fractions) for the active VIX bucket.      Buckets are pro, resolve_wing_widths_for_vix(), _active_widths_and_sigmas(), Shared entry selection for live trading and backtests., Resolve the widths to scan and their positional VIX sigmas. (+2 more)

### Community 61 - "Community 61"
Cohesion: 0.18
Nodes (5): DecisionQueries, Database query helpers for all tables., Queries for decision_log table., Queries for spot_prices table., SpotQueries
...
Cohesion: 0.2
Nodes (10): merge_chains(), Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains. (+2 more)

### Community 65 - "Community 65"
Cohesion: 0.2
Nodes (7): CandidateQueries, Queries for butterfly_candidates table., Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow., TradeService
...
Cohesion: 0.22
Nodes (8): Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars.

### Community 70 - "Community 70"
Cohesion: 0.22
Nodes (9): Use the first regular-session snapshot for gap direction., Use the first regular-session snapshot for gap direction., Use the first regular-session snapshot for gap direction., Use the first regular-session snapshot for gap direction., Use the first regular-session snapshot for gap direction., Use the first regular-session snapshot for gap direction., Use the first regular-session snapshot for gap direction., Use the first regular-session snapshot for gap direction. (+1 more)
...
Cohesion: 0.22
Nodes (6): Queries for tent_boundaries table., TentQueries, Execute SQL migration files in order., Run all SQL migration files in order., run_migrations(), main()
...
Cohesion: 0.29
Nodes (5): iter_chain_options(), Shared utilities for parsing Schwab option chain responses., Yield (strike, option_type, opt_dict) for each option matching the expiration., Extract the three butterfly leg quotes from the chain for position valuation., Extract the three butterfly leg quotes from the chain for position valuation.

### Community 81 - "Community 81"
Cohesion: 0.29
Nodes (5): fly_mark_value(), Pydantic models for option data and trade records., Butterfly value at mark: lower.mark - 2*center.mark + upper.mark., O(N*W) scan: for each center strike within spot_range, for each wing_width,, O(N*W) scan: for each center strike within spot_range, for each wing_width,
...
Cohesion: 0.33
Nodes (6): _parse_config_time(), Parse an HH:MM config time., Parse an HH:MM config time., Parse an HH:MM config time., Parse an HH:MM config time., Parse an HH:MM config time.
...
- **Why does `run_single()` connect `Community 27` to `Community 64`, `Community 1`, `Community 32`, `Community 39`, `Community 40`, `Community 41`, `Community 13`, `Community 47`, `Community 48`, `Community 17`, `Community 19`, `Community 23`, `Community 26`, `Community 60`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `ButterflyCandidate` connect `Community 4` to `Community 32`, `Community 65`, `Community 1`, `Community 3`, `Community 6`, `Community 72`, `Community 43`, `Community 13`, `Community 46`, `Community 16`, `Community 81`, `Community 49`, `Community 19`, `Community 20`, `Community 24`, `Community 30`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `OptionQuote` connect `Community 4` to `Community 0`, `Community 65`, `Community 3`, `Community 35`, `Community 5`, `Community 6`, `Community 42`, `Community 43`, `Community 15`, `Community 80`, `Community 81`, `Community 49`, `Community 18`, `Community 20`, `Community 22`, `Community 24`, `Community 26`?**
