...
## Corpus Check
- 110 files · ~282,455 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1594 nodes · 2692 edges · 111 communities (95 shown, 16 thin omitted)
- Extraction: 72% EXTRACTED · 28% INFERRED · 0% AMBIGUOUS · INFERRED: 744 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0c5aa7e0`
...
Cohesion: 0.05
Nodes (65): _profit_exit_reason(), BaseModel, CollectorSettings, DatabaseSettings, EntrySettings, ExecutionSettings, MonitoringSettings, ProfitManagementSettings (+57 more)
...
Cohesion: 0.06
Nodes (27): Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., BiasScoreFilter (+19 more)
...
Cohesion: 0.06
Nodes (38): chain_cache_path(), load_chain_day(), nearest_snapshot(), Real option chain cache — per-day JSON snapshots from the live collector.  Forma, Load all chain snapshots for a day.      Returns dict of UTC datetime -> list[Op, Return quotes from the most recent snapshot at or before bar_ts., Append one chain snapshot to the day's cache file.      Called by the collector, save_snapshot() (+30 more)
...
Cohesion: 0.09
Nodes (30): _butterfly_value(), _compute_spread(), detect_complete_days(), _elapsed(), _et(), LiveSpread, load_bars_from_db(), main() (+22 more)
...
Cohesion: 0.08
Nodes (34): main(), parse_args(), print_help(), Inspect what the strategy saw at entry for a given date.  Replicates the synthet, fmt_candidate(), load_bars_from_db(), load_chains_from_db(), main() (+26 more)
...
Cohesion: 0.12
Nodes (21): DayData, MinuteBar, Backtest data loader — fetches SPX 1-min bars and VIX from Polygon.io., _date_range(), ParameterSweeper, Parameter sweep engine for backtesting across date ranges., Runs grid search over simulation parameters across a date range., Run full parameter sweep. Returns a polars DataFrame. (+13 more)

### Community 14 - "Community 14"
Cohesion: 0.08
Nodes (16): PositionManager, Tracks position value from chain data and manages peak tracking., Reset for a new position. Optionally restore a persisted peak (e.g. after restar, Tracks position value from chain data and manages peak tracking., Reset for a new position. Optionally restore a persisted peak (e.g. after restar, DiscordNotifier, Discord webhook notifications., Sends trading notifications to Discord via webhook. (+8 more)
...
Cohesion: 0.09
Nodes (23): get_0dte_expiration(), minutes_since_open(), minutes_to_close(), now_eastern(), now_pacific(), Market timezone helpers for 0-DTE trading., Current time in US/Eastern., Current time in US/Pacific. (+15 more)
...
Cohesion: 0.11
Nodes (19): BaseSettings, AppConfig, OptionChainCollector, Option chain collector — fetches and stores SPX chain snapshots., Collects option chain snapshots at regular intervals., Parse Schwab callExpDateMap/putExpDateMap into flat rows., Integration tests for the option chain collector (requires live Schwab token)., A local JSON cache failure should not fail a DB-backed snapshot. (+11 more)
...
Cohesion: 0.16
Nodes (13): now_utc(), LiveSpread, OrderManager, Order execution with price ladder logic., Place one butterfly order at limit_price. Wait for fill; cancel if unfilled., Execute entry with price ladder: reprice from live mark each step,         step, Execute exit with reverse price ladder: reprice from live mark each step,, Manages order execution with price ladder and fill monitoring. (+5 more)
...
Cohesion: 0.16
Nodes (16): load_config(), Load configuration from YAML file and environment variables., Load configuration from YAML file and environment variables., Fetch and store daily OHLCV bars for SPX and VIX. Runs once per calendar day., Main collector loop — runs while market is open., str, Tests for configuration loading., Config values from YAML should override defaults. (+8 more)
...
Cohesion: 0.12
Nodes (10): Simulate one trading day., Simulate one trading day., Classify regime then delegate to simulate_day() with matching params.          R, Classify regime then delegate to simulate_day() with matching params.          R, Classify regime then delegate to simulate_day() with matching params.          R, Intraday VIX regime filter — skips entry when volatility is too elevated., Filter entries based on intraday VIX level at the time of entry., Most recent VIX bar close at or before entry_ts. None if no bars. (+2 more)
...
Cohesion: 0.13
Nodes (15): get_recent_closes(), merge_chains(), Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Up to *n* daily closes strictly before *date*, chronological order., Up to *n* daily closes strictly before *date*, chronological order. (+7 more)

### Community 40 - "Community 40"
Cohesion: 0.13
Nodes (15): get_vix_prev_close(), Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Cross-width selection. (+7 more)
...
Cohesion: 0.2
Nodes (12): DayResult, DrawdownWindow, Single-day simulation engine using synthetic option chains., Maps Regime → SimulationParams for use with simulate_day_adaptive().      Per-re, Maps Regime → SimulationParams for use with simulate_day_adaptive().      Per-re, RegimeDispatch, ButterflyCandidate, A butterfly spread candidate identified by the scanner. (+4 more)
...
Cohesion: 0.18
Nodes (10): OptionQuote, Pydantic models for option data and trade records., A single option quote from a chain snapshot., A trade record for tracking entry/exit., TradeRecord, load_chains_from_db(), Orchestrates the full entry/exit trading flow., Orchestrates the full entry/exit trading flow. (+2 more)
...
Cohesion: 0.2
Nodes (5): DecisionQueries, Database query helpers for all tables., Queries for decision_log table., Queries for tent_boundaries table., TentQueries
...
Cohesion: 0.25
Nodes (8): Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., select_for_width()
...
- **Why does `OptionQuote` connect `Community 46` to `Community 0`, `Community 3`, `Community 5`, `Community 38`, `Community 7`, `Community 8`, `Community 9`, `Community 14`, `Community 47`, `Community 18`, `Community 51`, `Community 23`, `Community 28`, `Community 61`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `TradeService` connect `Community 46` to `Community 1`, `Community 2`, `Community 34`, `Community 43`, `Community 13`, `Community 78`, `Community 47`, `Community 16`, `Community 14`, `Community 20`, `Community 22`, `Community 24`, `Community 56`, `Community 25`, `Community 59`, `Community 28`, `Community 61`, `Community 30`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 43` to `Community 1`, `Community 14`, `Community 19`, `Community 20`, `Community 22`, `Community 24`, `Community 25`, `Community 28`, `Community 30`, `Community 31`, `Community 34`, `Community 46`, `Community 58`, `Community 59`, `Community 61`, `Community 62`, `Community 68`, `Community 72`, `Community 75`, `Community 78`?**
