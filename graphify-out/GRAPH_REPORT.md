# Graph Report - butterfly-performance-followups  (2026-09-10)

## Corpus Check
- 278 files · ~328,159 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4028 nodes · 9652 edges · 230 communities (190 shown, 25 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 985 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e4409dae`
...
Cohesion: 0.09
Nodes (37): _butterfly_value(), _compute_spread(), detect_complete_days(), _elapsed(), _et(), get_prev_close(), get_vix(), LiveSpread (+29 more)
...
Cohesion: 0.07
Nodes (53): _easter_sunday(), get_us_market_early_closes(), get_us_market_holidays(), is_market_open(), is_trading_day(), _last_weekday(), market_close_time(), minutes_since_open() (+45 more)
...
Cohesion: 0.18
Nodes (7): _candidate_mark(), CandidatePaperExecutor, Any, Mark-price fills only; this object intentionally has no broker methods., MarketSnapshot, datetime, One atomically published, replayable view of candidate market data.

### Community 6 - "trade_chart.py"
Cohesion: 0.16
Nodes (23): build_exit_chart_png(), ButterflyChartSpec, candles_to_series(), _draw_strike_overlays(), _exit_chart_series(), _exit_marker_point(), _exit_window_end(), _fig_to_png() (+15 more)
...
Cohesion: 0.11
Nodes (14): Any, date, Schwab market-data client deliberately lacking every account/order operation., Prove the replacement credential with one bounded read-only Schwab call., Authenticate a Schwab client without resolving or retaining an account., Build one client with an isolated in-memory refresh-token callback., Validate and install a client built from a newly authorized token document., ReadOnlySchwabMarketDataClient (+6 more)

### Community 13 - "run_live.py"
Cohesion: 0.05
Nodes (63): BoundLogger, get_logger(), Structured logging setup with structlog., Configure structlog with JSON output and correlation IDs., Get a structlog logger with optional name., setup_logging(), clear_readiness(), Prometheus metrics for monitoring. (+55 more)
...
Cohesion: 0.09
Nodes (21): MinuteBar, BiasScoreFilter, Multi-signal directional bias filter for 0-DTE butterfly entries., High and low of the opening range (bars with ET time < 09:45). Edge case: no OR…, Exponential moving average seeded with SMA of first `period` bars. Returns None…, Scores market direction using 4 signals; returns CALL, PUT, or None., Compute bias score from 4 signals: gap : +1 if entry_close > prev_close, -1 if…, Volume-weighted average price using close as typical price. Edge case: all… (+13 more)

### Community 18 - "run_classifier_sweep.py"
Cohesion: 0.11
Nodes (26): DataFrame, CsvDataLoader, date, Path, CSV-based data loader for historical SPX + VIX 1-minute data. Reads two CSV…, Map each date → list of up to n prior daily closes (chrono order, newest last).…, Last VIX bar close per day as daily VIX proxy., Map each date → last close of the previous trading day. (+18 more)
...
### Community 23 - "notifier.py"
Cohesion: 0.10
Nodes (15): Lightweight Telegram and ButterflyGuy Alertmanager helpers. Usage: from…, Send a Telegram message. Returns True on success, False on failure., Post one stable, identifier-free alert fingerprint to Alertmanager., send(), send_alertmanager(), AlertmanagerNotifier, Trading and risk notifications., Sends centrally deduplicated critical alerts through Alertmanager. (+7 more)
...
Cohesion: 0.09
Nodes (5): Pool, SnapshotArchive, DatabasePool, Manages an asyncpg connection pool for TimescaleDB., Create the connection pool.
...
Cohesion: 0.17
Nodes (36): background_context(), _confirm_surfaces(), _filtered_gateway_logs(), _finite(), _gateway_error_code(), _health(), health_violations(), main() (+28 more)
...
Cohesion: 0.10
Nodes (15): AtomicSnapshotStore, Condition-guarded pointer swap; readers never observe partial snapshots., FakeArchive, FakeDb, FakeMarket, FakePool, asyncio, date (+7 more)
...
Cohesion: 0.06
Nodes (51): get_time_regime(), Classify minutes since open into a named time regime., _as_float(), _as_int(), Any, date, Convert option_chain_snapshots rows into OptionQuote objects., Build OptionQuote list from option_chain_snapshots query rows. (+43 more)
...
### Community 54 - "test_candidate_schwab_market_data.py"
Cohesion: 0.35
...
Cohesion: 0.08
Nodes (44): StrategySettings, main(), parse_args(), print_help(), Inspect what the strategy saw at entry for a given date. Replicates the…, Walk the 10:00-10:30 ET window and record every bar's entry signals., scan_entry_window(), EntryDecision (+36 more)
...
### Community 61 - "test_trade_chart.py"
Cohesion: 0.17
Nodes (15): entry_chart_window(), parse_hhmm(), date, time, Return [start_time - lookback, max(start_time, fill)] in the entry timezone., date, Tests for Discord trade chart generation., _sample_candles() (+7 more)
...
Cohesion: 0.10
Nodes (26): _age_seconds(), Any, date, datetime, Orchestrates the full entry/exit trading flow., Full entry flow from eligibility checks through entry fill., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's first regular-session open from Schwab intraday bars. (+18 more)
...
### Community 72 - "._retry"
Cohesion: 0.09
Nodes (12): date, Execute with exponential backoff retry., Fetch option chain for a specific symbol and expiration., Get current spot price for SPX., Get the status of an order., Cancel an existing order., Fetch 1-minute bars for today (and optionally prior days) from Schwab., Fetch 1-minute bars for one session. (+4 more)

### Community 73 - "core/config.py"
Cohesion: 0.08
Nodes (50): BaseSettings, ProfitManagementStrategy, AppConfig, CollectorSettings, ConfigModel, EntrySettings, ExecutionSettings, MonitoringSettings (+42 more)
...
Cohesion: 0.10
Nodes (28): nearest_snapshot(), Return quotes from the most recent snapshot at or before bar_ts., DayData, DayResult, datetime, Single-day simulation engine using synthetic option chains., Runs full strategy on a single day using synthetic options., Simulate one trading day. (+20 more)
...
Cohesion: 0.06
Nodes (81): DrawdownWindow, _asset_drawdowns(), backtest_entry_price(), candidate_from_trade_row(), _dd_schedule_label(), discover_dates(), _duration_min(), _find_bar_at() (+73 more)
...
### Community 103 - "send_test_chart.py"
Cohesion: 0.27
Nodes (9): _load_trade(), main(), Generate entry + EOD charts from a historic trade and post to Discord., load_spot_series(), date, Load spot price series from TimescaleDB for chart generation., spot_rows_to_candles(), build_entry_chart_png() (+1 more)
...
### Community 110 - "data_loader.py"
Cohesion: 0.14
Nodes (14): day_cache_path(), load_day(), _parse_bar(), date, Path, JSON cache helpers for DayData — shared across Schwab and future loaders., save_day(), Shared backtest market-data models. (+6 more)
...
Cohesion: 0.07
Nodes (50): canonicalize_schwab_chain_symbol(), DirectSchwabMarketDataProvider, _finite_number(), GatewayAuthoritativeMarketDataProvider, GatewayMarketDataError, _nonnegative_integer(), _now_eastern(), _optional_number() (+42 more)
...
### Community 140 - "evaluator.py"
Cohesion: 0.08
Nodes (33): assert_candidate_safety(), candidate_performance_stats(), CandidateAuditContext, CandidateDecisionQueries, CandidateEvaluator, CandidatePerformanceStats, config_sha256(), Path (+25 more)

### Community 141 - "walk_orders"
Cohesion: 0.38
Nodes (9): walk_orders(), _order_symbols(), _order(), test_redacted_audit_excludes_other_underlyings(), test_redacted_audit_reports_active_unknown_missing_and_duplicate_nodes(), test_redacted_audit_treats_replaced_as_historical_terminal(), test_order_symbols_walks_child_orders(), Any (+1 more)
...
### Community 150 - "_build_collector_market_data"
Cohesion: 0.20
...
Cohesion: 0.19
Nodes (11): _gateway_error(), asyncio, parametrize, test_gateway_error_code_reads_only_the_discriminator(), test_request_classifies_bare_504_without_error_body(), test_request_classifies_gateway_040_error_codes(), handler(), test_request_retry_does_not_retry_authorization_failure() (+3 more)
...
- **Why does `SchwabClientWrapper` connect `SchwabClientWrapper` to `Any`, `TradeService`, `run_morning_scan.py`, `._retry`, `GatewayAuthoritativeMarketDataProvider`, `record_equity_market_data.py`, `report_broker_order_statuses.py`, `ReadOnlySchwabMarketDataClient`, `run_live.py`, `_assert_broker_state_matches_db`, `position_service.py`, `evaluator.py`, `test_schwab_client.py`, `trade_service.py`, `services/daily_report_card.py`, `_build_collector_market_data`, `main`, `refresh_equity_universes.py`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `get_logger()` connect `run_live.py` to `run_paper_replay.py`, `feed.py`, `evaluator.py`, `ReadOnlySchwabMarketDataClient`, `MinuteBar`, `run_classifier_sweep.py`, `notifier.py`, `ProfitStateMachine`, `news.py`, `run_morning_scan.py`, `position_service.py`, `ButterflyCandidate`, `trade_service.py`, `StrategySettings`, `run_entry_analysis.py`, `ButterflyOrderBuilder`, `weekend_review.py`, `refresh_equity_universes.py`, `run_backtest_db.py`, `send_test_chart.py`, `data_loader.py`, `services/daily_report_card.py`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `AppConfig` connect `core/config.py` to `ProfitStateMachine`, `run_backtest_db.py`, `TradeService`, `entry_selection.py`, `test_candidate_executor.py`, `evaluator.py`, `run_live.py`, `CandidateRegistry`, `ValueError`, `position_service.py`, `test_candidate_evaluator_accounting.py`, `trade_service.py`, `test_collector.py`, `load_config`, `test_chain_parser_parity.py`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
