# Graph Report - butterflyguy  (2026-05-20)

## Corpus Check
- 110 files · ~282,472 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1596 nodes · 2696 edges · 121 communities (105 shown, 16 thin omitted)
- Extraction: 72% EXTRACTED · 28% INFERRED · 0% AMBIGUOUS · INFERRED: 746 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `86b738a0`
...
Cohesion: 0.06
Nodes (57): BaseModel, CollectorSettings, DatabaseSettings, EntrySettings, ExecutionSettings, MonitoringSettings, ProfitManagementSettings, QuoteQualitySettings (+49 more)
...
Cohesion: 0.05
Nodes (43): _fitted_density_counts(), _print_pnl_histogram(), print_thinkback_checklist(), Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Use the first regular-session snapshot for gap direction. (+35 more)
...
Cohesion: 0.08
Nodes (26): ButterflyCandidate, OptionQuote, Pydantic models for option data and trade records., A single option quote from a chain snapshot., A butterfly spread candidate identified by the scanner., A trade record for tracking entry/exit., TradeRecord, DecisionQueries (+18 more)
...
Cohesion: 0.12
Nodes (23): _butterfly_value(), detect_complete_days(), _elapsed(), _et(), load_bars_from_db(), main(), _mins_since_open(), _mins_to_close() (+15 more)
...
Cohesion: 0.1
Nodes (23): chain_cache_path(), ChainDay, load_chain_day(), nearest_snapshot(), Real option chain cache — per-day JSON snapshots from the live collector.  Forma, Load all chain snapshots for a day.      Returns dict of UTC datetime -> list[Op, Return quotes from the most recent snapshot at or before bar_ts., dict of {UTC datetime: OptionQuote list} with a pre-sorted key index for O(log n (+15 more)
...
Cohesion: 0.1
Nodes (18): _drawdown_rule(), _profit_exit_reason(), Simulate one trading day., Simulate one trading day., Simulate intraday using BS pricing, pinned to a pre-selected real entry., Simulate intraday using BS pricing, pinned to a pre-selected real entry., Simulate intraday using BS pricing, pinned to a pre-selected real entry., Classify regime then delegate to simulate_day() with matching params.          R (+10 more)
...
Cohesion: 0.18
Nodes (16): _dd_schedule_label(), _find_bar_at(), _live_width_label(), load_asset_config(), main(), nearest_snapshot(), parse_args(), _parse_dd_schedule() (+8 more)

### Community 23 - "Community 23"
Cohesion: 0.12
Nodes (18): get_time_regime(), Classify minutes since open into a named time regime., compute_tent_boundaries(), fly_bid_value(), fly_settlement_value(), Position value tracking and management., Calculate current butterfly value from latest chain quotes.         Value = lowe, Calculate current butterfly value from latest chain quotes.         Value = lowe (+10 more)
...
Cohesion: 0.18
Nodes (17): DayResult, DrawdownWindow, Single-day simulation engine using synthetic option chains., Maps Regime → SimulationParams for use with simulate_day_adaptive().      Per-re, Maps Regime → SimulationParams for use with simulate_day_adaptive().      Per-re, RegimeDispatch, SimulationParams, ProfitProtectorSettings (+9 more)
...
Cohesion: 0.12
Nodes (13): get_logger(), Structured logging setup with structlog., Configure structlog with JSON output and correlation IDs., Get a structlog logger with optional name., setup_logging(), Prometheus metrics for monitoring., Start the Prometheus metrics HTTP server., start_metrics_server() (+5 more)
...
Cohesion: 0.12
Nodes (16): Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Cross-width selection., Cross-width selection., Cross-width selection., Select the best butterfly candidate for a single wing width. (+8 more)
...
Cohesion: 0.13
Nodes (7): CandidateQueries, Database query helpers for all tables., Queries for tent_boundaries table., Queries for butterfly_candidates table., Queries for spot_prices table., SpotQueries, TentQueries

### Community 42 - "Community 42"
Cohesion: 0.13
Nodes (15): get_prev_close(), load_monitoring_chains(), Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly)., Load full-day snapshots for specific strikes only (3 legs of a butterfly). (+7 more)
...
Cohesion: 0.14
Nodes (14): get_recent_closes(), merge_chains(), Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Merge entry-window (all strikes) and monitoring (3 strikes, full day) chains., Up to *n* daily closes strictly before *date*, chronological order., Up to *n* daily closes strictly before *date*, chronological order. (+6 more)
...
Cohesion: 0.21
Nodes (9): _date_range(), ParameterSweeper, Parameter sweep engine for backtesting across date ranges., Runs grid search over simulation parameters across a date range., Run full parameter sweep. Returns a polars DataFrame., SweepConfig, Runs full strategy on a single day using synthetic options., Runs full strategy on a single day using synthetic options. (+1 more)
...
Cohesion: 0.17
Nodes (10): iter_chain_options(), Shared utilities for parsing Schwab option chain responses., Yield (strike, option_type, opt_dict) for each option matching the expiration., Parse Schwab chain response into OptionQuote objects., Parse Schwab chain response into OptionQuote objects., Parse Schwab chain response into OptionQuote objects., Parse Schwab chain response into OptionQuote objects., Parse Schwab chain response into OptionQuote objects. (+2 more)

### Community 52 - "Community 52"
Cohesion: 0.17
Nodes (12): get_vix_at(), load_bars_from_db(), load_date_data(), Load all data for one date. Returns None if insufficient data., Load all data for one date. Returns None if insufficient data., Load all data for one date. Returns None if insufficient data., Load all data for one date. Returns None if insufficient data., Load all data for one date. Returns None if insufficient data. (+4 more)
...
Cohesion: 0.18
Nodes (9): Full entry flow from eligibility checks through entry fill., Full entry flow from eligibility checks through entry fill., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars., Fetch today's first regular-session open from Schwab intraday bars. (+1 more)

### Community 55 - "Community 55"
Cohesion: 0.18
Nodes (11): _force_synthetic_for_date(), Patch load_chain_day to return None for `date`, forcing BS synthetic fallback., Inject DB chains into the chain cache for `date`. Returns restore callable., Patch load_chain_day to return None for `date`, forcing BS synthetic fallback., Patch load_chain_day to return None for `date`, forcing BS synthetic fallback., Patch load_chain_day to return None for `date`, forcing BS synthetic fallback., Patch load_chain_day to return None for `date`, forcing BS synthetic fallback., Patch load_chain_day to return None for `date`, forcing BS synthetic fallback. (+3 more)

### Community 56 - "Community 56"
Cohesion: 0.18
Nodes (11): _asset_drawdowns(), Resolve the DB connection string for local backtests.      Backtests follow the, Resolve the DB connection string for local backtests.      Backtests follow the, Resolve the DB connection string for local backtests.      Backtests follow the, Return live morning/late/afternoon drawdown thresholds., Return live morning/late/afternoon drawdown thresholds., Return live morning/late/afternoon drawdown thresholds., Resolve the DB connection string for local backtests.      Backtests follow the (+3 more)

### Community 57 - "Community 57"
Cohesion: 0.22
Nodes (9): _compute_spread(), EntryDecision, find_entry_candidate(), LiveSpread, Find best candidate in the 10:00–10:30 ET window, returning full decision contex, determine_direction(), DirectionFilter, Direction filter — determines CALL or PUT based on open vs previous close. (+1 more)
...
Cohesion: 0.29
Nodes (5): DayData, use_bias_filter=True should produce a trade result (direction set by bias)., direction_override takes precedence over use_bias_filter., When bias filter always returns None, day should be untraded., TestEngineIntegration

### Community 60 - "Community 60"
Cohesion: 0.31
Nodes (8): Trade service — orchestrates entry flow., Return the first regular-session open for the requested Eastern date., Return the first regular-session open for the requested Eastern date., _session_open_from_intraday_candles(), _candle(), test_session_open_ignores_premarket_and_missing_open_values(), test_session_open_returns_none_when_no_regular_session_bar_exists(), test_session_open_uses_first_regular_session_bar_for_requested_date()
...
Cohesion: 0.2
Nodes (10): _patch_chain_cache(), Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable., Inject DB chains into the chain cache for `date`. Returns restore callable. (+2 more)

### Community 64 - "Community 64"
Cohesion: 0.31
Nodes (5): Serves chain snapshots sequentially, advancing with each ladder step., replay_entry_ladder(), replay_exit_ladder(), ReplaySchwab, StepLog
...
Cohesion: 0.25
Nodes (8): discover_dates(), All dates in [start, end] with >= 50 snapshots for `underlying`., All dates in [start, end] with >= 50 snapshots for `underlying`., All dates in [start, end] with >= 50 snapshots for `underlying`., All dates in [start, end] with >= 50 snapshots for `underlying`., All dates in [start, end] with >= 50 snapshots for `underlying`., All dates in [start, end] with >= 50 snapshots for `underlying`., All dates in [start, end] with >= 50 snapshots for `underlying`.

### Community 76 - "Community 76"
Cohesion: 0.25
Nodes (8): get_vix_prev_close(), Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Up to *n* daily closes strictly before *date*, chronological order., Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars., Return VIX daily close strictly before *date* from daily_bars.

### Community 77 - "Community 77"
Cohesion: 0.32
Nodes (6): Helpers for choosing a candidate across multiple active widths., Choose the final candidate from one best candidate per width.      When `prefer_, select_cross_width_candidate(), _candidate(), test_cross_width_selection_can_prefer_first_bucket_width(), test_cross_width_selection_returns_none_for_empty_pool()
...
Cohesion: 0.5
Nodes (3): LiveSpread, Order execution with price ladder logic., NamedTuple

### Community 88 - "Community 88"
Cohesion: 0.5
Nodes (4): _parse_config_time(), Parse an HH:MM config time., Parse an HH:MM config time., Parse an HH:MM config time.
...
- **Why does `main()` connect `Community 6` to `Community 9`, `Community 16`, `Community 18`, `Community 25`, `Community 26`, `Community 27`, `Community 29`, `Community 30`, `Community 31`, `Community 32`, `Community 34`, `Community 35`, `Community 41`, `Community 57`, `Community 61`, `Community 62`, `Community 67`, `Community 74`, `Community 80`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `OptionQuote` connect `Community 6` to `Community 64`, `Community 0`, `Community 1`, `Community 68`, `Community 4`, `Community 7`, `Community 42`, `Community 13`, `Community 15`, `Community 17`, `Community 51`, `Community 21`, `Community 57`, `Community 27`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `ButterflyCandidate` connect `Community 6` to `Community 64`, `Community 0`, `Community 1`, `Community 13`, `Community 77`, `Community 16`, `Community 49`, `Community 86`, `Community 23`, `Community 25`, `Community 26`, `Community 27`, `Community 31`, `Community 57`?**
