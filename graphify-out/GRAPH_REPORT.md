...
## Corpus Check
- 106 files · ~280,564 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1398 nodes · 2407 edges · 107 communities (90 shown, 17 thin omitted)
- Extraction: 71% EXTRACTED · 29% INFERRED · 0% AMBIGUOUS · INFERRED: 689 edges (avg confidence: 0.7)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a3545d3c`
...
10. `TradeService` - 28 edges

## Surprising Connections (you probably didn't know these)
- `test_drawdown_rule_uses_legacy_regimes_by_default()` --calls--> `SimulationParams`  [INFERRED]
  tests/test_backtest_drawdown_schedule.py → src/butterfly_guy/backtest/simulation_engine.py
...
Cohesion: 0.07
Nodes (49): BaseModel, CollectorSettings, DatabaseSettings, EntrySettings, ExecutionSettings, MonitoringSettings, ProfitManagementSettings, QuoteQualitySettings (+41 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (48): BaseSettings, AppConfig, load_config(), Load configuration from YAML file and environment variables., OptionChainCollector, Option chain collector — fetches and stores SPX chain snapshots., Collects option chain snapshots at regular intervals., Integration tests for the option chain collector (requires live Schwab token). (+40 more)
...
Cohesion: 0.14
Nodes (30): StrategySettings, ButterflyCandidate, OptionQuote, Pydantic models for option data and trade records., A single option quote from a chain snapshot., A butterfly spread candidate identified by the scanner., EntryDecision, find_entry_candidate() (+22 more)
...
Cohesion: 0.08
Nodes (21): DayResult, _drawdown_rule(), DrawdownWindow, Single-day simulation engine using synthetic option chains., Simulate one trading day., Simulate intraday using BS pricing, pinned to a pre-selected real entry., Classify regime then delegate to simulate_day() with matching params.          R, Maps Regime → SimulationParams for use with simulate_day_adaptive().      Per-re (+13 more)

### Community 10 - "Community 10"
Cohesion: 0.11
Nodes (23): _butterfly_value(), _compute_spread(), detect_complete_days(), _elapsed(), _et(), load_bars_from_db(), load_chains_from_db(), main() (+15 more)
...
Cohesion: 0.1
Nodes (24): ButterflyOrderBuilder, Builds butterfly spread orders for Schwab API., Constructs Schwab-compatible butterfly order JSON., Build a butterfly BUY_TO_OPEN order., Build a butterfly SELL_TO_CLOSE order., LiveSpread, Order execution with price ladder logic., make_spx_candidate() (+16 more)
...
Cohesion: 0.13
Nodes (21): _dd_schedule_label(), discover_dates(), _find_bar_at(), main(), nearest_snapshot(), parse_args(), _parse_dd_schedule(), _patch_chain_cache() (+13 more)
...
Cohesion: 0.1
Nodes (15): PositionManager, Tracks position value from chain data and manages peak tracking., Reset for a new position. Optionally restore a persisted peak (e.g. after restar, Tracks position value from chain data and manages peak tracking., Reset for a new position. Optionally restore a persisted peak (e.g. after restar, PositionService, Position monitoring service — runs state machine on open positions., Monitors open positions and manages exits via the profit state machine. (+7 more)

### Community 24 - "Community 24"
Cohesion: 0.11
Nodes (17): A trade record for tracking entry/exit., TradeRecord, Full entry flow from eligibility checks through entry fill., Orchestrates the full entry/exit trading flow., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Parse Schwab chain response into OptionQuote objects., Fetch today's 1-min bars from Schwab and run BiasScoreFilter., Fetch today's 1-min bars from Schwab and run BiasScoreFilter. (+9 more)

### Community 25 - "Community 25"
Cohesion: 0.13
Nodes (15): Fetch and store daily OHLCV bars for SPX and VIX. Runs once per calendar day., Fetch current chain and store snapshot. Returns row count., Main collector loop — runs while market is open., Parse Schwab callExpDateMap/putExpDateMap into flat rows., Execute SQL migration files in order., Run all SQL migration files in order., run_migrations(), daily_reset_loop() (+7 more)
...
Cohesion: 0.2
Nodes (10): now_utc(), OrderManager, Place one butterfly order at limit_price. Wait for fill; cancel if unfilled., Execute entry with price ladder: reprice from live mark each step,         step, Execute exit with reverse price ladder: reprice from live mark each step,, Manages order execution with price ladder and fill monitoring., After a cancel, confirm the order didn't sneak through as filled., Poll order status until filled or timeout. (+2 more)
...
Cohesion: 0.14
Nodes (11): Select the best butterfly candidate for a single wing width., Select the best butterfly candidate for a single wing width., Cross-width selection., Cross-width selection., select_for_width(), select_live_width(), ButterflySelector, Butterfly selector — picks the best candidate from a list. (+3 more)
...
Cohesion: 0.14
Nodes (14): get_prev_close(), get_recent_closes(), get_vix_at(), get_vix_prev_close(), load_bars_from_db(), load_date_data(), Return the last spot price at or before 16:00 ET on the previous trading day., Return the last spot price at or before 16:00 ET on the previous trading day. (+6 more)
...
Cohesion: 0.2
Nodes (4): Queries for trades table., TradeQueries, dict, load_chains_from_db()
...
Cohesion: 0.2
Nodes (5): CandidateQueries, DecisionQueries, Database query helpers for all tables., Queries for decision_log table., Queries for butterfly_candidates table.
...
Cohesion: 0.25
Nodes (8): _fitted_density_counts(), _print_pnl_histogram(), Return bucket-height estimates from a Gaussian KDE fit., Return bucket-height estimates from a Gaussian KDE fit., Return bucket-height estimates from a Gaussian KDE fit., ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets., ASCII histogram with a fitted density curve overlaid on the trade buckets.
...
Cohesion: 0.5
Nodes (4): print_thinkback_checklist(), Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist., Print a per-trade ToS ThinkBack validation checklist.

### Community 70 - "Community 70"
Cohesion: 0.5
Nodes (4): _print_same_entry_comparison_table(), Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday, Print real vs same-entry-synthetic comparison (pinned center/price, BS intraday
...
Cohesion: 0.67
Nodes (3): _force_synthetic_for_date(), Patch load_chain_day to return None for `date`, forcing BS synthetic fallback., Patch load_chain_day to return None for `date`, forcing BS synthetic fallback.
...
- **Why does `OptionQuote` connect `Community 5` to `Community 0`, `Community 2`, `Community 4`, `Community 37`, `Community 41`, `Community 10`, `Community 46`, `Community 16`, `Community 21`, `Community 23`, `Community 24`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 25` to `Community 1`, `Community 5`, `Community 8`, `Community 9`, `Community 12`, `Community 18`, `Community 22`, `Community 23`, `Community 24`, `Community 29`, `Community 34`, `Community 39`, `Community 46`, `Community 48`, `Community 49`, `Community 50`, `Community 53`, `Community 61`, `Community 62`, `Community 68`, `Community 72`?**
  _High betweenness centrality (0.076) - this node is a cross-community bridge._
- **Why does `TradeService` connect `Community 24` to `Community 1`, `Community 34`, `Community 5`, `Community 8`, `Community 13`, `Community 46`, `Community 49`, `Community 18`, `Community 50`, `Community 52`, `Community 22`, `Community 25`, `Community 29`, `Community 62`?**
