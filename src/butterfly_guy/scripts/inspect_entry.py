...
def main() -> None:
    ...
    print(f"{'=' * 70}")
    print(
        f"  {'Strike':>7}  {'Mark':>7}  {'Bid':>7}  {'Ask':>7}  "
        f"{'IV':>6}  {'Delta':>7}  {'Dist':>6}"
    )
    print(f"  {'-' * 65}")
    for q in atm_quotes:
    ...
    def passes_filter(c):
        ...
        return (
            c.cost >= settings.min_debit
            and c.cost <= max_cost
            and c.reward_risk >= settings.rr_min
        )
    ...
    if not width_candidates:
        ...
        print(
            f"  {'Center':>7}  {'Lower':>7}  {'Upper':>7}  {'Cost':>6}  "
            f"{'MaxP':>6}  {'RR':>5}  {'Dist':>6}  {'LowerBE':>8}  "
            f"{'UpperBE':>8}"
        )
        print(f"  {'-' * 78}")
        ...
        for c in width_candidates:
            ...
            should_show = is_valid or (
                c.distance_from_spot < best_dist
                and c.distance_from_spot >= best_dist - 25
            )
            ...
            if is_selected:
                ...
                elif c.cost < settings.min_debit:
    ...
    if best:
        print(f"\n{'=' * 70}")
        print("  SELECTED BUTTERFLY")
        print(f"{'=' * 70}")
        print(f"  Direction:    {best.direction}")
        print(
            f"  Strikes:      {best.lower_strike:.0f} / {best.center_strike:.0f} / "
            f"{best.upper_strike:.0f}"
        )
        ...
        print(
            f"  Tent width:   {profit_tent_width:.2f} pts  "
            f"({best.lower_be:.2f} to {best.upper_be:.2f})"
        )
        print(
            f"  Distance:     {best.distance_from_spot:.2f} pts from spot "
            f"({entry_bar.close:.2f})"
        )
        print(
            f"\n  To profit at expiry, SPX must finish between {best.lower_be:.2f} "
            f"and {best.upper_be:.2f}."
        )
