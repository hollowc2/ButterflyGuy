...
async def test_extract_spot_price_agrees_with_the_live_client_on_the_same_payload(
    payload: dict[str, Any], expected: float
) -> None:
    """Differential test against SchwabClientWrapper.get_spot_price.

    ``data/schwab_client.py`` is not modified to share this helper, so the duplication is
    pinned here instead: both must derive the same price from the same payload.
    """
    ...
    # `client` is a read-only property; set the attribute it reads.
    wrapper._client = _FakeClient()  # type: ignore[attr-defined]

    live_price = await wrapper.get_spot_price("$SPX")
