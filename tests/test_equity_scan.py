...
def _quote_payload(
    *,
    ...
    if extended_last is not None:
        payload["extended"] = {"lastPrice": extended_last}
    return payload
...
def test_parse_equity_quote_uses_extended_price_for_gap():
    snap = parse_equity_quote(
        "AAPL",
        _quote_payload(close=100.0, last=98.0, net_pct=-2.0, volume=1_000_000, extended_last=105.0),
        universes={"sp500"},
    )
...
def test_build_snapshots_filters_by_price_and_volume():
    settings = EquityScanSettings()
    settings.filters.min_price = 10.0
    settings.filters.min_volume = 500_000
    symbol_map = build_symbol_map(
        {
            "custom": ["GOOD", "CHEAP", "LOWVOL"],
        }
    )
    quotes = {
        "GOOD": _quote_payload(close=100, last=112, net_pct=12.0, volume=1_000_000),
