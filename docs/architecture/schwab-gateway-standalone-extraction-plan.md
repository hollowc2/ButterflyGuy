...
- [ ] Run standalone for seven consecutive days including one full market session (`COMPLETE`
  `2026-08-18`) and one scheduled token reauthorization/reload lineage cycle (`COMPLETE`
  `2026-08-22`, zero restarts, lineage `2026-08-22T15:48:31Z`).
...
- Recovery-image loss root cause: `IDENTIFIED`. The earlier record that deletion time and cause
  ...
  and applied that change at `2026-08-18T04:11:36.012305190Z` from an interactive terminal, since
  the root-owned file cannot be edited from an agent session; the original is preserved at
  `/root/docker-image-prune.bak-20260818T040006Z`, outside `/etc/cron.d/` so cron never parses it.
  The installed line is `36 0 * * * root docker image prune -f --filter "until=24h"` with
  `root:root` ownership and mode `644` unchanged. Verified effect: `0` dangling images exist, so the
  next firing is a no-op, and all six recovery/release tags report `dangling=0`, which makes them
  structurally ineligible for the corrected prune even after a future application recreate orphans
  their images. The reclaimable `12.53GB` of build cache is untouched by either variant.
...
- Scope: repair the stale host virtualenv and establish a replacement application recovery
  ...
  networking, any running/stopped container, or the standalone/legacy images.
- Dependency correction: record the host package inventory, then install only
  `schwab-token-store` and `schwab-gateway-sdk` version `0.1.0` from the exact locked upstream
  commit `2d1da47b37ba48e3603f8d52a2fe73a55924aaf0`, without dependency upgrades. Require both host
  imports, the unchanged runtime imports, and clean source. Do not run the Discord sender
  manually or credit a manually generated archive.
- Dependency validation: let the next scheduled hourly keepalive run, require one authenticated
  refresh success and no new import/auth/lock/persistence failure, then prove unchanged token
  lineage/protection/six-view identity/lock, zero lifecycle changes, green monitoring, and a flat
  serialized broker audit. If validation fails, uninstall only the two newly added packages,
  re-prove the unchanged active runtime, record the failure, and stop.
- Recovery-image correction: because the four historical image IDs are absent, do not reuse their
  historical tag names or claim exact restoration. Add new timestamped Phase 8 recovery tags to
  the three exact currently running SPX/NDX/XSP image IDs, verify the mappings, and retain the
  historical loss as evidence. Do not build, pull, delete, restart, recreate, or retag any other
  image/container. If validation fails, remove only the newly created tags, revalidate the active
  images, record the failure, and stop.
- Reset boundary: after both corrections and all safety gates pass, record that timestamp as a
  fresh qualifying start with a new seven-day earliest end. Observe a new full regular session
...
- Proposed window: Saturday `2026-08-22T16:15:00Z` (`09:15 PDT`), while the market is closed,
  after corrective validation and preferably after a replacement full-session proof, but before
  the baseline lineage's seven-day deadline. The non-hour boundary avoids the normal hourly
  keepalive slot and restores a Saturday-morning cadence with recovery time.
- Current packet baseline: lineage `2026-08-16T23:34:35Z`; protected regular document mode
  ...
  duplicate IDs `0`. The missing recovery images make this packet stale: every fact, the repaired
  host keepalive, and the replacement recovery baseline must be re-proved immediately before the
  window; this packet does not authorize proceeding on stale evidence.
...
- Acceptance dependency: this packet cannot become ready for approval until the replacement
  qualifying clock reaches its newly recorded seven-day end without reset, the replacement
  full-session and approved scheduled
...
| 2026-08-17 | Phase 8 checkpoint 2 follow-up / market session | IN PROGRESS | fixed-output interval `2026-08-17T18:35:11.363397Z`–`18:51:45.810953Z` (`994` seconds): clean source and all `18` exact resource baselines unchanged; `12` direct log sources / `1,911` bounded records; lifecycle events and every required anomaly category `0`; Prometheus target/token-ready interval minima `1/1`; gateway alerts `0` | Observed: all health/readiness, exposure/security, paper/live/direct/shadow/XSP-alias, protected-token, and unchanged-resource gates passed. The `2` remaining open production rows are expected current-date, single-quantity, distinct-asset `mark_v1` paper positions; unsafe/duplicate/wrong-date opens, nonterminal intents, interval non-mark fills, and broker intents are `0`. The paired serialized broker audit returned SPX/NDX/XSP positions `0/0/0`, active orders `0`, missing/unmapped statuses `0/0`, and duplicate IDs `0`; lineage remains `2026-08-16T23:34:35Z`, mode/owner and all six views remain correct, and the lock is available. The source-correct session audit reports zero production/candidate fill, settlement, schedule, data, parity-failure, risk-state, or position-limit failures; the `2` open rows and one SPX EOD-chart flag are expected before close. Reporting configuration is present exactly once for the daily and live-performance jobs and twice for keepalive; no report failures are recorded, while today's live page and archive are correctly not yet current/present before their `20:30Z` and `22:00Z` schedules. Inference: continuity and session behavior remain on track; no close, settlement, EOD, or report credit is taken early. |
| 2026-08-17 | Phase 8 checkpoint 2 pre-close uniqueness | IN PROGRESS | fixed-output interval `2026-08-17T18:51:45.810953Z`–`19:09:38.499307Z` (`1,072` seconds): clean source; all `18` exact ID/image/start/restart/state baselines unchanged and each Compose project/service identity occurs exactly once across `26` total containers; `12` direct log sources / `2,057` bounded records; lifecycle events and all required anomaly categories `0`; Prometheus target/token-ready interval minima `1/1`; gateway alerts `0` | Observed: every endpoint, exposure/security invariant, paper/live/direct/shadow/XSP-alias guard, and database safety gate passed; the `2` open rows remain expected current-date, single-quantity, distinct-asset `mark_v1` paper positions, with unsafe/duplicate/wrong-date opens and broker intents `0`. An ordinary atomic persistence advanced the protected document inode to `3413` with mtime `2026-08-17T19:00:31.423020Z`; lineage remains `2026-08-16T23:34:35Z`, mode/owner and six identical views remain correct, and the lock is available. Inference: service uniqueness and continuity remain clean; the persistence is not credited as the scheduled reauthorization cycle, and no close, settlement, EOD, or report credit is taken early. |
| 2026-08-17 | Phase 8 checkpoint 2 pre-close exposure | IN PROGRESS | fixed-output interval `2026-08-17T19:09:38.499307Z`–`19:13:18.136051Z` (`219` seconds): all `18` exact baselines and Compose uniqueness unchanged; all `12` active app/feed/gateway endpoints are monitoring-network-only, every published binding is loopback-only, and the candidate feed remains unpublished; `12` direct log sources / `430` bounded records; lifecycle events and every required anomaly category `0`; Prometheus interval minima `1/1`; alerts `0` | Observed: endpoints, runtime guards, gateway security, token protection/lineage/views/lock, and database safety all pass; the `2` open rows remain expected safe paper positions and broker intents remain `0`. Inference: no network-exposure or continuity regression is present; close, settlement, EOD, and report credit remain pending. |
| 2026-08-17 | Phase 8 checkpoint 2 pre-close broker scope | IN PROGRESS | serialized seven-day broker read: SPX/NDX/XSP positions `0/0/0`; active orders `0`; same-day target-underlying orders `0`; missing/unmapped statuses `0/0`; duplicate/missing order IDs `0/0`; target-order timestamps ambiguous `0` | Observed: the audit covers terminal as well as active same-day SPX/NDX/XSP order strategies, so a filled or canceled target order cannot disappear behind a flat end state. The paired token check through `2026-08-17T19:18:41.549098520Z` preserved lineage `2026-08-16T23:34:35Z`, mode/owner, six identical views, and lock availability. Inference: there is no aggregate broker evidence of real Butterfly-underlying activity; market close and final settlement/report gates remain pending. |
| 2026-08-17 | Phase 8 checkpoint 2 pre-close lineage enforcement | IN PROGRESS | fixed-output interval `2026-08-17T19:13:18.136051Z`–`19:25:26.169366Z` (`728` seconds): all exact baselines, Compose uniqueness, endpoint exposure, and runtime guards unchanged; `12` direct log sources / `1,395` bounded records; lifecycle events and every anomaly category `0`; Prometheus minima `1/1`; alerts `0` | Observed: the checkpoint now rejects rather than merely reports any lineage other than the explicitly expected value; it also fails closed on token type, symlink, mode, owner, six-view identity, and lock state. The hardened gates pass with lineage `2026-08-16T23:34:35Z`; an ordinary atomic persistence advanced the inode to `3779` with mtime `2026-08-17T19:21:15.521477Z` while preserving all invariants. The `2` remaining opens are still safe current-date paper positions and broker intents remain `0`. Inference: no unauthorized lineage or continuity change occurred; this ordinary persistence is not the scheduled cycle, and close/report credit remains pending. |
| 2026-08-17 | Phase 8 checkpoint 2 rollback retention | IN PROGRESS | fixed-output interval `2026-08-17T19:25:26.169366Z`–`19:28:06.622069Z` (`160` seconds): all exact resources, uniqueness, exposure, health/readiness, runtime, lifecycle, log, monitoring, token, and database gates pass; all `4` retained application rollback tags are present and resolve to their exact recorded image IDs | Observed: standalone and legacy images also remain anchored by their unchanged exact containers. The shared document persisted atomically again at `2026-08-17T19:25:38.522838Z`, changing inode to `876` while retaining the exact baseline lineage, protection, six identical views, and available lock. Inference: no rollback evidence or runtime continuity was lost; close/report credit remains pending. |
| 2026-08-17 | Phase 8 checkpoint 2 retained-image tags | IN PROGRESS | fixed-output interval `2026-08-17T19:28:06.622069Z`–`19:48:48.248756Z` (`1,241` seconds): all `18` exact resources and Compose identities, `12` endpoint exposures, health/readiness, runtime guards, lifecycle, logs, monitoring, token, and database gates pass; the `4` application rollback tags and `3` retained gateway-image tags resolve to their exact recorded image IDs | Observed: `schwab_gateway_live:v0.1.0` and `schwab_gateway_candidate-live:latest` both resolve to standalone image `sha256:d31e679d15e60e3ca8b794a4ff22ed4c742836cabd102656121a90630ab5ef04`; `butterfly_gateway_foundation-schwab_gateway_live:latest` resolves to legacy image `sha256:6eb9f560effae529a2f578b5a4e5a1b0da2fd124cb4566fe9b097f01ec8b0ec8`. Lifecycle events and every anomaly category are `0`; Prometheus interval minima are `1/1`; token lineage/protection/six views/lock remain exact. The `2` open rows remain expected safe paper positions, with no unsafe opens or broker intents. Inference: release and recovery-image retention is now exact rather than container-anchor-only; close, settlement, EOD, and report credit remain pending. |
| 2026-08-17 | Phase 8 checkpoint 2 post-close / live report | IN PROGRESS | fixed-output interval `2026-08-17T19:48:48.248756Z`–`21:15:07.079205Z` (`5,178` seconds): all exact resources, tags, exposure, health/readiness, runtime, lifecycle, logs, monitoring, token, and database gates pass; open trades/nonterminal intents/broker intents are `0/0/0`; lifecycle events and every anomaly category are `0`; Prometheus interval minima are `1/1` with no alerts | Observed: all `3` production paper trades and `5` candidate paper trades are closed. Source-correct entry/exit fill, mark-v1, window, settlement, schedule, decision, data, parity-failure, risk-state, P&L, halt, position-limit, secondary-work, and EOD-chart failure counts are all `0`; the two non-failing parity observations remain recorded. The live-performance artifact has scheduled mtime `2026-08-17T20:30:03.368965Z`, valid shape and same-session SPX data, exact public-HTTPS byte identity, a success event, and no later failure. The paired serialized broker audit returned positions `0/0/0`, active and same-day target-underlying orders `0/0`, missing/unmapped statuses `0/0`, duplicate/missing IDs `0/0`, and ambiguous timestamps `0`; lineage remains exactly `2026-08-16T23:34:35Z` through `21:16:31.160271Z`, with protected six-view identity and lock availability. Inference: market operation, settlement, EOD, live reporting, and broker safety are clean; full-session credit still awaits the scheduled daily archive. |
...
| 2026-08-20 | Phase 8 day 3 | COMPLETE | daily report archive `reports/daily_report_card/2026-08-20.md` (`401` bytes) written `22:00`; equity morning scan produced `2026-08-20.json`/`.md` at `13:00Z`; three trades all `CLOSED` | Observed: NDX `249` `-4.62` `drawdown_afternoon`, XSP `250` `+1.45` `cash_settled`, SPX `251` `-0.73` `drawdown_morning`. DB open trades and nonterminal broker intents `0`. Lineage held at `2026-08-16T23:34:35Z`. The report's watchlist line "Balance change differs from trade P&L by `$-2,161.80`" is the co-tenant TQQQ/turtlequant activity in the same account view, not a butterfly reconciliation defect. Live-performance page regeneration was not verified for this day and is not credited. |
| 2026-08-20 | Deviation — accidental full docker daemon restart inside the qualifying window | ACCEPTED as clock-compatible (Corey, `2026-08-22`) | every container on Helios restarted simultaneously at `2026-08-20T02:39:49.88Z`–`02:39:51.50Z`, and `docker.service` reports `ActiveEnterTimestamp=2026-08-20 02:40:38 UTC`; cause confirmed by the operator as accidental, not a scheduled or approved action | Observed: what survived — every container ID, every `Created` timestamp (SPX/NDX/XSP `2026-08-17T00:3x`, gateway `2026-08-19T18:26`), every image ID, and `RestartCount=0` on all containers, since a daemon restart does not increment that counter. What did not survive — `StartedAt` advanced on all `18` baselined containers, and every process was stopped and started, so all in-memory state including cached Schwab clients was rebuilt. Two readings are available and they differ by a week: (a) clock-compatible, by the same reasoning that credited the `2026-08-19T18:26Z` gateway rebuild — no resource identity changed and no configuration was mutated; or (b) not clock-compatible, because `RestartCount=0` is an artifact of the daemon-restart path rather than evidence of process continuity, and a seven-day stability soak whose subject processes all restarted on day 2 has not demonstrated what it claims. Under (b) the qualifying start moves to `2026-08-20T02:39:51.499039813Z` and the earliest end to `2026-08-27T02:39:51.499039813Z`. Decision, `2026-08-22`: Corey accepted reading (a) — the deviation is clock-compatible, the qualifying start stays `2026-08-18T04:00:06.893767848Z`, and the earliest end stays `2026-08-25T04:00:06.893767848Z`. Recorded as a deliberate operator judgment, not an oversight, and taken with reading (b) stated above in full. The counter-argument is preserved verbatim so a later reader can re-weigh it: no evidence here demonstrates process continuity across `2026-08-20T02:39:5xZ`, and `RestartCount=0` does not supply it. |
| 2026-08-21 | Phase 8 day 4 | COMPLETE | daily report archive `reports/daily_report_card/2026-08-21.md` (`405` bytes) written `22:00`; equity morning scan produced `2026-08-21.json`/`.md` at `13:00Z`; three trades all `CLOSED` | Observed: SPX `252` `-0.94` `drawdown_morning`, XSP `253` `-0.28` `cash_settled`, NDX `254` `-4.67` `drawdown_afternoon`. DB open trades and nonterminal broker intents `0`; all six consumers running at `RestartCount=0`; SPX/NDX/XSP `/ready` `200`; both gateways `healthy`. Live-performance page regeneration was not verified for this day and is not credited. NDX has now lost on ten consecutive trades, and `consecutive_loss_warning` reports `action=trading_allowed_review_recommended` — see the `2026-08-19` observation on the structurally unreachable daily-loss limit. |
| 2026-08-22 | Phase 8 scheduled reauthorization / lineage cycle | COMPLETE | new lineage `2026-08-22T15:48:31Z` (Sat), derived expiry `2026-08-29T15:48:31Z`; installed inode `818`, `787` bytes, mode `600`, owner `1001:1001`, digest `c443417680b7`; all six token consumers agree with the host on one inode and one digest; `RestartCount=0` and `StartedAt` unchanged at `2026-08-20T02:39:5xZ` on every consumer | Observed: this closes the Phase 8 checklist item "one scheduled token reauthorization/reload lineage cycle". Preconditions at `15:45:36Z`: `0` rows with null `exit_time`, market closed, six consumers running at restart `0`, local gate `632 passed, 1 skipped` and Ruff clean. Minted on zeus into a scratch path, mode corrected `644`→`600` before transfer, staged byte-identical (`c443417680b7` on both sides), moved under `flock -w 30 .tokens.json.lock` with a stamped pre-replacement backup; inode advanced `1766`→`818`. All four cached consumers reloaded with no restart and no container identity change: feed `candidate_market_data_token_reloaded` + `candidate_token_reload_applied` at `2026-08-22T15:56:12.683Z`/`.691Z`, then SPX/NDX/XSP `schwab_token_reloaded=1` each with `failed=0` between `15:56:06Z` and `15:57:09Z`. Post-move: both gateways `healthy`, SPX/NDX/XSP `/ready` `200`, `schwab_token_persist_failed` and `task_group_error` absent. Neither gateway required action; each builds and discards a client per request inside the token lock. Next deadline Saturday `2026-08-29T15:48Z`. |
| 2026-08-22 | Verification correction — `docker logs --since` is unusable on the candidate feed | CORRECTION | `docker logs --since 15m butterfly_spx_candidate_feed` returned `0` lines while `--tail` showed live access-log entries through the same instant; the `--since` reload count read `0` continuously from `15:55:02Z` to `16:03:29Z` for a reload that had already succeeded at `15:56:12.683Z` | Observed: this is a fourth verification correction alongside the three recorded `2026-08-19`, and the same class as the recorded `docker events` defect and the checkpoint-0 finding that the log API "returned immediately for `tail=0` but timed out before one record for `tail=1`". The runbook's step-4 rule — no reload after 6+ minutes, then `docker restart` — was wired to this sensor, so a mechanical reading would have burned the window with ~2.5 days remaining on a consumer that was already healthy. Corrected in `docs/runbooks/reauthorization-2026-08-22-checklist.md`: `--tail` replaces `--since`, both feed success events are named with their source lines, and the bare 6-minute trigger is replaced by a confirm-by-second-method rule that also requires proving the query can return a positive for that container before believing a negative. Deferring past the gate is not an alternative: the prior refresh token expires `2026-08-23T23:34Z`, `1 d 4 h` before the gate and `14.5 h` before Monday's open. |
| 2026-08-22 | Qualifying-start ambiguity in this ledger | OBSERVATION | the `2026-08-16` Phase 8 baseline row still carries qualifying start `2026-08-17T00:39:37.451595824Z`, which the "Phase 8 stability ledger" section header invalidates in favour of `2026-08-18T04:00:06.893767848Z` (earliest end `2026-08-25T04:00:06.893767848Z`) | Observed: the invalidated start is stated only in the section header, while the superseded row remains `IN PROGRESS` and reads as current. Reading the row alone yields a gate of `2026-08-24T00:39:37Z` — a full day early — which inverts the safety analysis around the token expiry at `2026-08-23T23:34Z`: against the false gate the prior token appears to die `65` minutes before close, against the true gate it dies `1 d 4 h` before close and a full live session intervenes. Any deferral or restart decision priced against the row rather than the header is wrong in the unsafe direction. Read the qualifying start from the section header. |
| 2026-08-22 | Transient keepalive `invalid_grant`, self-recovered | OBSERVATION | the `19:00Z` scheduled keepalive returned `ERROR: unsupported_token_type: 400 Bad Request` with `error=invalid_grant`, `"Refresh token is invalid, expired or revoked"`; the `20:00Z` run succeeded with one authenticated refresh and SPX quote `200`, reporting `163.8h` remaining | Observed: `163.8h` at `20:00Z` reconciles exactly with the `2026-08-22T15:48:31Z` mint plus `168h`, so the new refresh token was and is valid; the failure did not reflect the document's state. No consumer saw it — `"level": "error"` count `0` across SPX/NDX/XSP for the surrounding three hours, `schwab_token_persist_failed` `0`, gateway clean, all six consumers running at `RestartCount=0`. The token document held inode `818` across the interval while its digest moved `c443417680b7`→`d446a03dc5a6`, which is the keepalive rotating the access token by in-place write; the apps' atomic persistence advances the inode instead (`876`→`3842` in the `2026-08-16` baseline), so no application persisted during this window. Assessed as a transient upstream rejection: it recovered unaided within one cycle and left no trace in any consumer. Recorded rather than absorbed because the Phase 8 acceptance criteria include no auth degradation, and one unexplained `invalid_grant` inside the window is evidence a reader is entitled to weigh. A recurrence should be treated as degradation, not noise. |
